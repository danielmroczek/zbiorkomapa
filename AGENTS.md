...
- All Linia/Kierunek assembly decisions (direction grouping, route merging, representative-trip/dominant shape, is_loop, on-demand, audio_id, shape fallback) live in `scripts/route-assembler.js` (`assemble(bundle, { audio, routeSegments })`) over a plain-row seam. `scripts/processor.js` is only the node-gtfs adapter (`fetchCityBundle`) + persistence — it fetches rows and writes outputs, it does not decide. "Prefer fixing the shared processor logic once" means fixing the assembler once.
